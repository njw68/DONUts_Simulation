...
function [bots,I] = GeoSensorEPMPositions(bots,I,currentBot)
    %%The angles where a red dot will be plotted on each circle to represent
    %%the location of the EPMs 
        bots(currentBot).geometry(:,1) = round((bots(currentBot).coordinates(1) + cos(I.t1)),4);
        bots(currentBot).geometry(:,2) = round((bots(currentBot).coordinates(2) + sin(I.t1)),4);
    for k = 1:I.numEPMs
