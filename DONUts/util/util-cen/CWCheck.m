function [bots,world,moves,I] = CWCheck(bots,world,I,moves,bot,bot_rotate,both,Tri,sensorPos,magpos)
%Centralized CWCheck
...
for i=1:length(Tri)
    ...
    if ~isempty(botObs)
        [inbotObs,onbotObs] = inpolygon(botObs(:,1),botObs(:,2),xtri,ytri);
    end
    ...
    if ~isempty(world.obstacles)
        ...
        for obsNum = 1:length(I.seenObs)
            ...
            obsPoly = polyshape(px,py);
            collide = intersect(Tri(i),obsPoly);
            ...
            if collide.NumRegions ~= 0
                ...
                if magpos ==1
                    ang(1) = angles(moverMagPos(magpos)); ang(2) = angles(moverMagPos(end-1)); %CW angles
                elseif magpos ==2
                    ang(1) = angles(moverMagPos(magpos)); ang(2) = angles(moverMagPos(end)); %CW angles
                else
                    ang(1) = angles(moverMagPos(magpos)); ang(2) = angles(moverMagPos(magpos-2)); %CW angles
                end

                %creates the CW checked region about the point of rotation
                CWx = [C_rotateX (C_rotateX + (radius)*cosd(ang(1))) (C_rotateX + (radius)*cosd(ang(2))) C_rotateX];
                CWy = [C_rotateY (C_rotateY + (radius)*sind(ang(1))) (C_rotateY + (radius)*sind(ang(2))) C_rotateY];
                roTri = polyshape(CWx,CWy);
